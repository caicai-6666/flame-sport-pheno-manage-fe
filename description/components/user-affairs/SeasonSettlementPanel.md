...
待终审接口携带 `season_user_id`、`project_id` 关联键，并在补传资格存在时携带资格创建时固化的初审上下文快照。页面按以下规则组合展示：

- 从 `userProfileCatalog` 读取已经由结算参与者接口缓存的姓名、头像地址与用户主键，不重复调用用户详情接口。
- 从当前参赛记录的项目集合读取项目名称，关联缺失时拒绝展示不完整队列。
- 补传记录直接使用快照中的 `levelId` 和 `ruleContent` 展示资格建立时的目标要求，不受后续全局规则修改影响。
- 历史无快照记录才从完整挑战等级目录恢复 `level_id`，再以 `project_id + level_id` 加载全局目标要求并复用 `projectRuleCatalog` 缓存。
- 快照存在但项目归属、等级或规则结构异常时拒绝展示整批队列，不用当前全局规则掩盖数据错误。
