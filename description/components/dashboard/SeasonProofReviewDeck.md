...
- 打开单条凭证时通知调用方加载 `projectId + levelId` 对应规则，并展示加载、失败、空规则或有序指标状态。
- 在固定高度视窗中独立滚动并缩放凭证图片。
- 为通过和拒绝提供限时二次确认，确认完成后规范化管理员评语并通过接口提交终审结果。
- 提供管理员评语伸缩控制，临时收起输入区但保留已输入内容和两个审核按钮。
- 审核后播放对应离场动画并返回列表。

组件通过专用 API 模块提交终审结果，但不自行计算项目进度回退、回补或排行榜变化；这些事务规则由后端负责。
...
手写评语会去除首尾空白，未填写时分别使用“凭证符合项目要求，终审通过。”和“凭证不符合项目要求，终审未通过。”。
...
项目等级规则由调用方按需请求。组件根据当前记录的 `ruleKey` 读取状态；规则成功后逐项展示 `label` 与 `value`，并通过轻微渐显避免指标突然跳出。相同组合的缓存与请求合并由 `projectRuleCatalog` 负责。
