...
`SeasonProofReviewDeck` 是当前赛季卡片背面的运动记录终审工作区，用于先浏览待审列表，再进入单条凭证完成通过或拒绝操作。
...
- 以宽松的大尺寸列表展示全部待审核记录，并突出用户、项目、日期、挑战要求和备注摘要。
- 点击列表项后进入对应记录的单条审核卡片，允许管理员按需选择审核顺序。
- 在列表、审核卡片和完成状态之间使用淡入、位移、轻微缩放与模糊过渡，避免内容直接替换。
- 在固定高度的独立视窗中展示纵向拼接凭证，允许审核员滚动查看完整图片。
- 悬浮或键盘聚焦凭证顶部的用户项目标签时，展示该用户的挑战等级和当前项目目标要求。
- 将用户备注和模型评语收纳为纵向排列的两个图标，悬浮或键盘聚焦后通过磨砂气泡展示完整内容。
- 将管理员评语输入框与通过、拒绝操作组合成唯一独占整行的底部回复编辑器，并随审核决定一并提交。
- 提供“通过”和“拒绝”两个明确的终审动作。
- 审核后播放方向不同的离场动画，移除已处理记录并返回待审列表。
...
| `reviewed` | `{ recordId, reviewStatus, reviewComment }` | 当前卡片离场动画结束、返回待审列表前 |
...
组件初始显示待审列表，不会自动打开第一条记录。管理员点击任意列表项后进入单条审核卡片；卡片中的返回按钮只返回列表，位于列表或完成状态时点击返回才退出整个终审工作区。

列表条目采用较大的行高、头像和文字层级，列表本身独立滚动。列表与卡片切换时，旧视图先短暂淡出，新视图再以较慢的上浮动画进入；系统启用减少动态效果时会关闭这些过渡。

管理员必须先填写终审评语，才能执行通过或拒绝；未填写时组件显示校验提示并将焦点移入输入框。点击审核按钮后，输入框和按钮会暂时禁用，防止动画期间重复提交；当前记录离场后触发 `reviewed`，由调用方从队列中移除已处理记录，组件随后回到更新后的列表。

凭证图片铺满整个卡片底层。用户备注和模型初审评语收纳在底部左侧上下排列的两个图标中，默认只占用紧凑的图标空间。鼠标悬浮或键盘聚焦图标时，右侧展开对应的完整磨砂气泡；移开或失焦后自动收起，避免两段较长文字同时挤占图片区域。

管理员回复编辑器是唯一独占整行的区域，固定在卡片最底部，并使用轻度磨砂背景承载输入框及通过、拒绝按钮。长图不再附加额外滚动占位，滚动到底时图片底边与卡片底边对齐，并由卡片圆角严格裁切，不会超出展示范围。

凭证顶部的用户、项目与日期标签支持鼠标悬浮和键盘聚焦。信息浮层只用于补充审核上下文，不遮挡标签本身；挑战等级来自用户的赛季级选择，目标要求则应由当前项目与该等级对应的项目规则生成。

凭证图片区具有独立的纵向滚动上下文。图片起始位置贴齐展示框顶部，滚动到末尾时贴齐底部；滚动位置被限制在有效范围内，触控惯性到达边缘时不会继续越界或露出框外区域。鼠标滚轮、触控手势和键盘滚动只移动凭证图片，人员信息、审核依据与操作按钮保持在卡片中可见。每次从列表打开记录时，卡片重新挂载，因此滚动位置从图片顶部开始。
