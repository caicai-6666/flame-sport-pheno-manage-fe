...
- 使用顶部胶囊导航替代传统左侧菜单，提供数据看板、平台配置和用户事务三个主模块入口的视觉位置。
...
组件当前不负责路由切换、接口请求、权限判断或服务端业务状态管理。终审结果当前只在本地原型数据中推进。
...
当前赛季区域使用两个稳定卡面组成的三维工作区。每次切换时，目标页面会先装入隐藏面，容器再累计旋转 `180°`，因此赛季概览、终审卡组、奖品列表和意见列表之间都能连续翻页。未显示的一面通过 `inert` 禁止焦点和点击进入，避免误操作隐藏内容。
...
- 待发放奖品与新用户意见来自本地演示数组。奖品条目可以通过“已发放”按钮从本地队列清除并同步更新数量；用户意见仍只提供列表预览。
...
- 终审卡组：`src/components/dashboard/SeasonProofReviewDeck.vue`
