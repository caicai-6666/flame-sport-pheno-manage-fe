...
- 使用四周留白、圆角边框和柔和阴影建立独立的应用窗口感；外壳以珊瑚色与深海蓝径向拼接纹理替代纯白底，并由圆角裁切在工作台内部。70% 不透明度的顶部栏、看板卡片与待办项相应提高为高不透明度表面，确保纹理不影响信息阅读。
...
- 待终审、待发放奖品或用户意见成功处理一项后，数量按个位、十位和百位拆成独立滚轮槽；只有发生变化的位数向上滚出并从下方接入新数字，未变化位保持静止。例如 `21 → 20` 只滚动个位，`10 → 09` 同时滚动两位。数位槽按全局手绘字体放大后的字面宽高预留空间，数字及错误态横杠不会越出数量区域；滚轮不会重复播放初次加载动画。
