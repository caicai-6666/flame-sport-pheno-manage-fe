...
- 使用四周留白、圆角边框和柔和阴影建立独立的应用窗口感；工作台外壳使用 78% 冷白半透明表面，让公司分子场在留白区域轻微透出。内部卡片维持约 87%～89% 冷白背景以保护数据可读性，顶部栏使用 82% 冷白背景。工作台和顶部栏不对持续变化的 Canvas 执行大面积 `backdrop-filter`，避免每帧触发昂贵的背景重采样。
- 认证完成后由应用先预载两张异步图表卡片；看板首次挂载时以不可见但参与布局的状态等待关键卡片实例和首轮容器测量全部完成，再整体显现，避免用户看到网格现场重排。
...
- 工作台根节点声明分子背景交互屏蔽边界；鼠标位于外壳内部时，背景只保留自主呼吸，不响应指针扰动。
...
认证成功或缓存会话恢复后，调用方先通过 `dashboardLayoutPreloader` 等待决定看板网格结构的两个异步图表卡片完成解析，再挂载工作台。看板挂载后使用 `visibility: hidden` 保留真实尺寸，不设固定帧数上限地等待两个关键卡片实例、Vue DOM 更新及浏览器首轮几何布局全部就绪，随后才整体显现。该门禁只覆盖组件解析和容器布局；当前赛季、项目、待办及图片接口仍在工作台挂载后独立加载，并在完整容器内展示各自加载态。
