...
登录或会话恢复成功后，应用先预载决定首屏网格结构的图表组件，期间登录卡片保持可见并显示“正在生成工作台布局”。预载成功后才挂载工作台；工作台在不可见状态完成关键容器的首轮布局，再触发整体进入效果。赛季、项目、待办和图片等业务请求不属于布局门禁，仍在完整容器内独立展示加载状态。
