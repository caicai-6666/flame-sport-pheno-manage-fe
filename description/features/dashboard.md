...
- 管理员进入工作台后自动获取激活赛季。
- 进入工作台前预载异步布局组件，并在隐藏挂载阶段确认关键卡片和首轮容器布局全部生成后再显现看板；业务数据继续由各区域独立加载。
...
```mermaid
flowchart TD
  A[管理员通过认证] --> A1[保持登录视图并预载异步看板布局组件]
  A1 --> A3[隐藏挂载并等待全部关键容器完成首轮布局]
  A3 --> A4[显现看板并保留各卡片加载态]
  A4 --> B[GET season-statistics/current]
  B -->|200| C[校验并适配赛季响应]
  C --> D[渲染当前赛季卡片]
...
- 看板显现条件只包含异步布局组件解析、关键卡片实例挂载、DOM 更新和浏览器首轮布局，不设置固定帧数超时，也不等待赛季、项目、待办、用户或图片接口返回。
