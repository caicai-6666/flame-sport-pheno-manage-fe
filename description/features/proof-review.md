...
当前阶段只在数据看板中提供**逐张卡片审核原型**，尚未接入后端列表与提交接口。

---

## 原型入口与流程

管理员点击“今日待办”中的“待终审记录”后，当前赛季卡片翻转到终审面板。面板每次聚焦一条运动记录，并在其后方预览下一张卡片。
...
每完成一条原型记录，工作台会从本地队列中移除该记录，数据看板中的待终审数量同步减一。切换到奖品或意见页面后再次进入终审，也会继续处理剩余卡片。
...
- 审核结果只推进前端卡组并递减本地数量，刷新页面后恢复。
- 没有真实修改 `proof_record.review_status`。
- 没有执行项目进度回退、回补或排行榜更新。
- 暂不支持拖拽手势，当前通过两个审核按钮触发左右离场动画。
