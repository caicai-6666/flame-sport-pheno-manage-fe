...
燃动现象智能管理平台前端是面向企业运动赛季运营人员的 Web 应用，用于查看赛季数据、维护平台配置、审核运动凭证、完成赛季积分结算，以及处理奖品发放和用户意见。
...
| Canvas 2D | 登录页与工作台共用的交互式公司分子场背景 |
...
登录页与工作台共用一套以公司官网 [PhenoSolar](https://www.phenosolar.com/) 当前首页效果为基准的 Canvas 2D 分子场。桌面端使用 300 个分子和 800px 能量半径，移动端使用 80 个分子和 400px 能量半径；鼠标或自动游走的能量中心通过同一径向波驱动分子位移、键长、尺寸、绿蓝色带与辉光。工作台采用冷白半透明外壳和更高不透明度的业务卡片，让背景主要出现在留白区域。分子计算和绘制运行于 `OffscreenCanvas Worker`，主线程只节流转发鼠标位置；不支持该能力的浏览器使用静态近白背景。
